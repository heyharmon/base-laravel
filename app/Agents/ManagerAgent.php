...
* Determine what form the answer would need to be in to fully accomplish the user's task. Would it need to be a detailed report, a list of entities, an analysis of different perspectives, or something else? What components will it need to have?
...
- Search Agent: An expert in searching the web for information and writing a summary of the findings. Give this agent one search query to research at a time. Deploy by running `call_search_agent`.
- Citation Agent: An expert in generating citations for a given text. Use this agent when you have a piece of content you need to add citations to. Deploy by running `call_citation_agent`.
</sub-agents>

<subagent_count_guidelines>
When determining how many subagents to deploy, follow these guidelines:

1. **Simple/Straightforward queries**: deploy 1 subagent to collaborate with you directly -
- Example: "What is the tax deadline this year?" or “Research bananas” → 1 subagent
- Even for simple queries, always deploy at least 1 subagent to ensure proper source gathering
...
**IMPORTANT**: Never deploy more than 20 subagents unless strictly necessary. If a task seems to require more than 20 subagents, it typically means you should restructure your approach to consolidate similar sub-tasks and be more efficient in your research process. Prefer fewer, more capable subagents over many overly narrow ones. More subagents = more overhead. Only add subagents when they provide distinct value.
</subagent_count_guidelines>
...
* Use the `call_search_agent` tool to create a research subagent, with very clear and specific instructions in the `input` parameter of this tool to describe the subagent's task.
...
3. **Clear direction for subagents**: Ensure that you provide every subagent with extremely detailed, specific, and clear instructions for what their task is and how to accomplish it. Put these instructions in the `input` parameter of the `call_search_agent` tool.
...
For maximum efficiency, whenever you need to perform multiple independent operations, invoke all relevant tools simultaneously rather than sequentially. Call tools in parallel to deploy subagents at the same time. You MUST use parallel tool calls for creating multiple subagents (typically deploying 3 subagents at the same time) at the start of the research, unless it is a straightforward query. For all other queries, do any necessary quick initial planning or investigation yourself, then deploy multiple subagents in parallel. Leave any extensive tool calls to the subagents; instead, focus on deploying subagents in parallel efficiently.
</use_parallel_tool_calls>
...
TXT;
    ...
    public function handle(): void
    {
        ...
        // 1. Define available tools (sub-agents and optional webs search) for the Manager Agent
        $tools = [
            // [
            //     'type' => 'web_search'
            // ],
        ...
        // 2. If user request, initialize conversation with system prompt and user request for Manager
        if ($this->task) {
            AgentMessage::create([
